...
router.post('/', async (req, res) => {
  ...
  console.log('Image mime type:', req.body.imageMimeType ?? 'none');
  ...
  try {
    const {
      ...
      userProfile,
      medicalRecords,
      imageBase64,   // legacy field (full data URI)
      image,         // new field: pure base64 string
      imageMimeType, // new field: mime type string
    } = req.body;
    ...
    // ── Resolve file data (support both legacy data-URI and new pure-base64) ─
    let fileData: string | null = null;
    let fileMime: string | null = null;

    if (image && imageMimeType) {
      // Strip any accidental data-URI prefix the frontend may have left in
      fileData = image.includes(',') ? image.split(',')[1] : image;
      fileMime = imageMimeType;
    } else if (imageBase64) {
      // Legacy: full data URI like "data:image/png;base64,iVBOR..."
      if (imageBase64.includes(',')) {
        fileMime = imageBase64.split(';')[0].split(':')[1];
        fileData = imageBase64.split(',')[1];
      } else {
        fileData = imageBase64;
        fileMime = 'image/png'; // fallback
      }
    }

    if (fileData) {
      console.log(`Adding ${fileMime} attachment (${Math.round(fileData.length / 1024)} KB base64)`);
    }
